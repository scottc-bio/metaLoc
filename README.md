...
## Core workflow
Protein sequences are the input for the core workflow. For both eukaryotic and prokaryotic sequences the workflow utilises [SignalP 6.0](https://www.nature.com/articles/s41587-021-01156-3) ([Source code](https://github.com/fteufel/signalp-6.0) in `euk` mode for eukaryotic sequences and in `other` mode for prokaryotic sequences for signal peptide prediction. Next [DeepLoc 2.1](https://academic.oup.com/nar/article/52/W1/W215/7642068) ([Source code](https://github.com/teevee112/DeepLoc-2.0) is utilised for eukaryotic protein localisation prediction, and [DeepLocPro 1.0](https://academic.oup.com/bioinformatics/article/40/12/btae677/7900293) ([Source code](https://github.com/Jaimomar99/deeplocpro) for prokaryotic protein localisation prediction. Finally, transmembrane helices prediction is performed by [Phobius 1.01](https://academic.oup.com/nar/article/35/suppl_2/W429/2920784) ([Server](https://phobius.sbc.su.se/data.html) or [DeepTMHMM 1.0](https://www.biorxiv.org/content/10.1101/2022.04.08.487609v1) ([Server](https://services.healthtech.dtu.dk/services/DeepTMHMM-1.0/). 
...
## Meta mode
In `--meta` mode, protein sequences can be predicted from the nucleotide sequences. [AUGUSTUS 3.5.0] ([Source code](https://github.com/Gaius-Augustus/Augustus)(https://academic.oup.com/bioinformatics/article/24/5/637/202844) utilised the user-selected model to predict genes from eukaryotic contigs. [Prodigal 2.6.3](https://link.springer.com/article/10.1186/1471-2105-11-119) ([Source code](https://github.com/hyattpd/Prodigal) performs ORF prediction from prokaryotic contigs. For sequences of unknown origin or from mixed community sample, [EukRep 0.6.7](https://pmc.ncbi.nlm.nih.gov/articles/PMC5880246/) ([Source code](https://github.com/patrickwest/EukRep) can be utilised for life domain classification of contigs into eukaryotic or prokaryotic sequences. Both branches of the workflow are then performed simultaneously.
...
On the system used to develop and test this workflow, the test profile took 6 minutes and 41 seconds to complete. This included time for the initial automatic conda environment setup, which is cached and re-used in for future runs. For example, running a second time without the need for environment setup took only 1 minute and 46 seconds.
...
| signalp_CS_Position | Both | SignalP prediction of signal peptide cleavage site |
| deeplocpro_Localization | Prokaryotic | DeepLocPro prediction of prokaryotic protein localisation |
| deeploc_Localizations | Eukaryotic | DeepLoc prediction of eukaryotic protein localisation |
| deeploc_Signals | Eukaryotic | DeepLoc prediction of eukaryotic protein signal peptide |
| deeploc_Membrane_types | Eukaryotic | DeepLoc prediction of eukaryotic protein membrane type |
| phobius_TM | Both (if deepTMHMM not enabled) | Phobius prediction of number of transmembrane helices |
| phobius_SP | Both (if deepTMHMM not enabled) | Phobius prediction of signal peptide |
| phobius_PREDICTION | Both (if deepTMHMM not enabled) | Phobius prediction transmembrance helices topology |
| deeptmhmm_Num_TM_helices | Both (if deepTMHMM enabled) | DeepTMHMM prediction of number of transmembrane helices |
| deeptmhmm_TM_helices(start-end) | Both (if deepTMHMM enabled) | DeepTMHMM prediction of transmembrane helices start and end positions |
| deeptmhmm_Predicton | Both (if deepTMHMM enabled) | DeepTMHMM prediction of protein type |
| deeptmhmm_Topology | Both (if deepTMHMM enabled) | DeepTMHMM prediction of protein sequence topology |
...
# Metagenomic application

Applying this workflow to a set of 96,091 mixed microbial metagenomic contigs resulted in the prediction of 1,380,482 protein coding sequences. The entire workflow from metagenomic contigs to protein characteristic prediction and results merging took 10 days 13 hours 23 minutes and 20 seconds to complete.
